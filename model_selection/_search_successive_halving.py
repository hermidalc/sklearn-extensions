...
class _SubsampleMetaSplitter:
    ...
    def split(self, X, y, **cv_params):
        for train_idx, test_idx in self.base_cv.split(X, y, **cv_params):
            train_idx = resample(
                train_idx,
                replace=False,
...
class ExtendedBaseSuccessiveHalvingCV(ExtendedBaseSearchCV):
    ...
    def _check_input_parameters(self, X, y, **cv_params):
        ...
        if isinstance(self, ExtendedHalvingRandomSearchCV):
        ...
        if self.min_resources_ in ("smallest", "exhaust"):
            if self.resource == "n_samples":
                n_splits = self._checked_cv_orig.get_n_splits(X, y, **cv_params)
    ...
    def fit(self, X, y=None, **fit_params):
        ...
        **fit_params : dict of string -> object
        ...
        self._check_input_parameters(X=X, y=y, **cv_params)

        self._n_samples_orig = _num_samples(X)

        super().fit(X, y=y, **cv_params, **fit_params, **feature_params)
...
class ExtendedHalvingGridSearchCV(ExtendedBaseSuccessiveHalvingCV):
...
class ExtendedHalvingRandomSearchCV(ExtendedBaseSuccessiveHalvingCV):
