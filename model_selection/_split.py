    ...
    def split(self, X, y, groups, weights=None):
        ...
        if weights is None:
            indices = (
                ...
                .groupby("group")
                .apply(pd.DataFrame.sample, n=1, random_state=self.random_state)
            ...
            indices = (
                pd.DataFrame({"group": groups, "weight": weights})
                .groupby("group")
                .apply(
                    pd.DataFrame.sample,
                    n=1,
        ...
        for _ in range(self.n_splits):
            if weights is None:
                ...
                indices = (
                    pd.DataFrame({"group": groups, "weight": weights})
                    .groupby("group")
                    .apply(pd.DataFrame.sample, n=1, random_state=rng, weights="weight")
                    .reset_index(level=0, drop=True)
                    .sort_index()
                    .index.values
                )
