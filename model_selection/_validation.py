...
def _check_routing(param_routing):
...
def cross_validate(
    ...
    *,
    ...
    """
    router = _check_routing(param_routing)
...
def _fit_and_score(
    ...
    try:
        ...
        fit_time = time.time() - start_time
        test_scores = _score(
            estimator, X_test, y_test, scorer, error_score, score_params
        )
        score_time = time.time() - start_time - fit_time
        if return_train_score:
            train_scores = _score(
                estimator, X_train, y_train, scorer, error_score, fit_params
            )
...
def _score(estimator, X_test, y_test, scorer, error_score="raise", score_params=None):
...
def cross_val_predict(
    ...
    *,
    ...
    cv = check_cv(cv, y, classifier=is_classifier(estimator))
    splits = list(cv.split(X, y, **cv_params))
...
def permutation_test_score(
    ...
    *,
    ...
    """
    estimator_is_list = bool(type(estimator) in (list, tuple, set, frozenset))
    base_estimator = estimator[0] if estimator_is_list else estimator
    ...
    cv = check_cv(cv, y, classifier=is_classifier(base_estimator))
    scorer = check_scoring(base_estimator, scoring=scoring)
    ...
    if estimator_is_list and len(estimator) != cv.get_n_splits():
        raise ValueError(
            "If number of passed estimators > 1 then must equal " "number of CV splits"
        )
    ...
    permutation_scores = Parallel(n_jobs=n_jobs, verbose=verbose)(
        delayed(_permutation_test_score)(
            clone(estimator),
            X,
            shuffle_y(y, groups, random_state),
            cv,
            scorer,
            cv_params,
            fit_params,
...
def _permutation_test_score(
    estimator, X, y, cv, scorer, cv_params, fit_params, score_params, feature_params
):
    ...
    score_params = score_params if score_params is not None else {}
    scores = []
    for cv_idx, (train, test) in enumerate(cv.split(X, y, **cv_params)):
        # Subset fit_params values for train indices
        train_fit_params = _check_fit_params(X, fit_params, train)
        train_fit_params = {**train_fit_params, **feature_params}
        # Subset score_params values for test indices
        test_score_params = _check_fit_params(X, score_params, test)
        test_score_params = {**test_score_params, **feature_params}
        estimator_instance = estimator[cv_idx] if estimator_is_list else estimator
        X_train, y_train = _safe_split(estimator_instance, X, y, train)
        X_test, y_test = _safe_split(estimator_instance, X, y, test, train)
        estimator_instance.fit(X_train, y_train, **train_fit_params)
        scores.append(scorer(estimator_instance, X_test, y_test, **test_score_params))
...
def _incremental_fit_estimator(
    ...
    for n_train_samples, partial_train in partitions:
        # Subset fit_params values for partial_train indices
        partial_fit_params = _check_fit_params(X, fit_params, partial_train)
        partial_fit_params = {**partial_fit_params, **feature_params}
        ...
        if y_partial_train is None:
            partial_fit_func(X_partial_train, **partial_fit_params)
        else:
            partial_fit_func(X_partial_train, y_partial_train, **partial_fit_params)
        fit_time = time.time() - start_fit
        fit_times.append(fit_time)

        start_score = time.time()

        train_score_params = {**partial_fit_params, **feature_params}

        test_scores.append(
            _score(estimator, X_test, y_test, scorer, error_score, score_params)
        )
        train_scores.append(
            _score(estimator, X_train, y_train, scorer, error_score, train_score_params)
        )
